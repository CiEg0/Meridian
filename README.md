...
| 模式 | 行为 | 建议 |
|---|---|---|
| `host` | 在共享入口按精确 `public_host` 分流，不绑定站点保留端口 | **推荐**；面板应绑定回环地址，或配置精确的 `TRUSTED_PROXY_CIDRS` |
| `port` | 只监听站点独立端口 | 适合简单部署；独立端口会绑定所有接口，应配合防火墙 |
| `both` | 同时开放共享域名和独立端口 | 仅用于兼容旧客户端；属于显式高风险模式 |

`public_host` 只接受精确 DNS 名称，不支持 scheme、端口、路径、IP 或通配符。Meridian 不支持路径前缀部署。

### 回源与播放分流

| 配置 | 用途 |
|---|---|
| `target_url` | 网页、API、元数据和默认媒体回源 |
| `playback_target_url` | 可选的播放、转码和下载回源 |

不设置播放回源时，所有请求使用主回源。手工播放列表最多保存 128 个受信任 authority：`direct` 模式使用第一个地址，`redirect` 模式则只跟随精确匹配列表的 GET/HEAD 301、302、307、308；它不是轮询、负载均衡或自动故障转移。
