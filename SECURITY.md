...
- 密码使用 bcrypt 哈希存储
- 首次管理员用户名要求 1-64 个 UTF-8 字节，密码要求 12-72 个 UTF-8 字节；管理页面在提交前执行相同边界校验并要求再次确认密码
...
- 首次创建管理员必须在启动前提供 `SETUP_TOKEN`；服务本身不会生成、记录或通过管理 API 返回令牌，创建成功后令牌会从进程内存清除。一键安装器会将随机令牌写入受保护的 `.env`，新生成时只显示一次；已有令牌不会自动回显，尚未完成初始化的管理员可由 root 从该文件恢复。管理 UI 只有在认证状态接口明确返回待初始化时才显示创建流程；状态检查失败时禁止提交并提供重试
...
- 管理员显式选择“重定向跟随”并配置播放回源时，手工规则独立于自动发现 Profile 和已知播放路径分类：只对非 Upgrade 的 GET/HEAD 处理 301/302/303/307/308，最多 5 跳，每一跳必须精确匹配配置的播放 authority，并检测循环。手工 authority 可为管理员明确选择的 HTTP、私网或非标准端口目标，但不会扩展动态发现规则；跨 authority 仍执行媒体 Header 白名单和敏感信息清洗。CONNECT、WebSocket/Upgrade、`/_meridian/d/` capability 路径、POST/PUT/DELETE 及请求正文不跟随，即使站点同时启用 Extreme 也不会向手工目标重放正文
...
- 策略、解析、DNS、拨号或 TLS 失败返回清洗后的 502；容量/速率返回 503 和有限 Retry-After。上游 manifest 的 4xx/5xx 保留状态码但正文替换为固定 manifest 错误对象；动态 resource/redirect 的上游错误正文同样替换为固定动态错误对象。所有错误路径只透传经过 24 小时界限验证的单一 `Retry-After`，且失败请求不刷新 capability idle 时间。客户端响应、错误和日志不包含 target path/query、RequiredHeaders 或 capability。播放路由观察表只聚合规范化 authority、来源、目标类型（同权威/手工配置/自动发现）、决定、由有限原因确定的处理阶段、3xx 状态、时间和次数；协议拒绝进一步区分 request_unclassified、structured_body_limit、playback_info_denied、hls_feature_denied、dash_feature_denied、redirect_body_replay_denied，不保存 URL、query、DNS 回答、Header 或正文。2,048 项异步队列过载只增加进程全局 dropped 计数；记录保留 30 天、每站点最多 500 行且全库最多 10,000 行
